...
class commonHelper{
	...
	public static function checkUserPassPortCategoryacordingToCountry($country_id){
		...
		if(in_array($country_id,$RequireVisa)){

			$visa_category = 'Visa Needed';

		}elseif(in_array($country_id,$restricted)){

			$visa_category = 'Restricted Country';

		}elseif(in_array($country_id,$doNotRequireVisa)){
	...
	public static function getVolunteerRole($id){
		
		$role = explode(',',$id); $data = '';

		if(!empty($role) && count($role)>0){

			foreach($role as $val){

				$roleData = \App\Models\Role::where('id',$val)->first();
				$data.= $roleData->name.', ';
			}
