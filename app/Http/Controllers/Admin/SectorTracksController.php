...
class SectorTracksController extends Controller {
    ...
    public function add(Request $request) {
		...
		if($request->ajax() && $request->isMethod('post')){
			...
			if ($validator->fails()){
                ...
                if ((int) $request->post('id') > 0) {
                    $data=\App\Models\SectorTracks::find($request->post('id'));
                } else {
                    $data=new \App\Models\SectorTracks();
                }	
                ...
                if ((int) $request->post('id') == 0) {
                    return response(array('message'=>'Sector Tracks added successfully.', 'reset'=>true),200);
                } else {
                    return response(array('message'=>'Sector racks updated successfully.','reset'=>false),200);
                }
        ...
        return view('admin.Sector_Tracks.add', compact('result','users'));
    ...
    public function list(Request $request) {
		...
		if ($request->ajax()) {
			...
			$query = \App\Models\SectorTracks::orderBy($order,$dir);
			...
			$totalData1 = \App\Models\SectorTracks::orderBy($order,$dir);
        ...
        return view('admin.Sector_Tracks.list');

	}

    public function edit(Request $request,$id) {
		
		$result = \App\Models\SectorTracks::find($id);
		...
		return view('admin.Sector_Tracks.add')->with(compact('result'));
    ...
    public function delete(Request $request, $id) {
		...
		if ($result) {
			...
			\App\Models\SectorTracks::where('id', $id)->forceDelete();
			$request->session()->flash('success','Sector Tracks deleted successfully.');
	...
	public function status(Request $request) {
		...
		$result = \App\Models\SectorTracks::find($request->post('id'));

		if ($result) {
			$result->status = $request->post('status');
			$result->save();

			return response(array('message'=>'Sector Tracks status changed successfully.'), 200);
		} else {
	...
	public function getGroupUsersList(Request $request) {
		...
		$data = \App\Models\SectorTracks::where('id', $request->post('id'))->first();
