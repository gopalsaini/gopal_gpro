...
<ul>
   ...
   @if(!$SpouseInfoResult)
        ...
        <li>
            <a href="@if($stage->stage > 1) {{url('payment')}} @else # @endif" class="@if($stage->stage > 1) active @endif">
    ...
    <li>
        <a href="@if($stage->stage > 4) {{url('event-day-information')}} @else # @endif" class="@if($stage->stage > 4) active @endif">
            <span>0@php echo $i++; @endphp</span>@lang('web/profile.event') @lang('web/profile.day')
        </a>
    </li>
