...
@section('title',__('Regional Affinity'))
    ...
    <div class="page-header">
        <div class="row">
			...
			<div class="col-sm-6">
				<div class="bookmark">
					<ul>
						<a href="{{ route('admin.regional_affinity.add')}}" class="btn btn-primary"><i class="fas fa-list me-2"></i> Add</a>
					</ul>
				</div>
			</div>
        </div>
...
$(document).ready(function() {
    ...
    var table = $('#tablelist').DataTable({
        ...
        "ajax": {
            "url": "{{ route('admin.regional_affinity.list') }}",
            "dataType": "json",
            "async": false,
            "type": "get",
            data: function (d) {
    ...
    $('#tablelist tbody').on('click', '.group-user-list', function () {
        ...
        if (row.child.isShown()) {
            ...
            $.post("{{ route('admin.regional_affinity.get-user-data') }}", { _token: "{{ csrf_token() }}", id: email }, function(data) {
...
function fill_datatable() {
    $('.-change').change(function() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('admin.regional_affinity.status')}}",
            headers: {
