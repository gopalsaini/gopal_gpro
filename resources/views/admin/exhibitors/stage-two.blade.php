                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <div class="form-line">
                                <label for="inputName">@lang('admin.offer') <label class="text-danger">*</label></label>
                                <select id="offer_code" class="form-control">
                                    <option value="">--Select One--</option>
                                    @if (count($offers) > 0)
                                        @foreach ($offers as $offer)
                                            <option value="{{$offer->code}}">{{$offer->code}}</option>
                                        @endforeach
                                    @else
...
function fill_datatable() {
    $('.-change').click(function() {
        ...
        if (status === 1) {
            ...
            var amount = null;
            var offer_code = null;
            statusChange(id, status, amount, offer_code);
            return true;
        }
    });
}

function statusChange(id, status, amount, offer_code) {
    $.ajax({
        type: "POST",
        dataType: "json",
...
function clickOnStatusChangeBtn() {
    ...
    if (id !== 0 && amount != '' && offer_code != '') {
        statusChange(id, 1, amount, offer_code);
        modalHide();
    } else if (amount == '') {
        sweetAlertMsg('error', '403 : The amount field is required');
    } else if (offer_code == '') {
        sweetAlertMsg('error', '403 : The amount offer code field is required');
    } else {
